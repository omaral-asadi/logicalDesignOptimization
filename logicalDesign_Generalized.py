...
# Defines set of external input values for each scenario
externalInputValues = {
    "A": [0,0,1,1],
    "B": [0,1,0,1],
}

# Defines set of external output values for each scenario
externalOutputValues = [0,1,1,0]
...
def solveModel():
    ...
    # Adds logic gate usage and external output connection  variables to model respective
    x = model.addVars(logicGates, vtype=GRB.BINARY, name="x")
    t = model.addVars(logicGates, vtype=GRB.BINARY, name="t")
    ...
    # Adds constraints for correspondance of external output
    for i in logicGates:
        for j in scenarios:
            model.addConstr(y[i,j] <= externalOutputValues[j] + (1 - t[i]), "externalOutputCorrespondance_1[%s,%s]"%(i,j))
            model.addConstr(y[i,j] >= externalOutputValues[j] - (1 - t[i]), "externalOutputCorrespondance_2[%s,%s]"%(i,j))

    # Requires connection of at least one gate to external output
    model.addConstr(quicksum(t[i] for i in logicGates) == 1, "externalOutputConnectionRequirement")
    ...
    for i in logicGates:
        ...
        for k in logicGates:
            ...
            if k!=i:
                ...
                for n in gateInputs[i]:
                    model.addConstr(x[i] >= 1 - (1 - v[k,i,n]), "gateUsageImplication_1[%s,%s,%s]"%(k,i,n))
    ...
    for i in logicGates:
        ...
        model.addConstr(
            ...
            ) + t[i] >= 1 - (1 - x[i]), "gateOutputDecisionRequirement[%s]"%i)
    ...
    model.optimize()
    return model, x, t, v, u, y, w, s

# Defines function for displaying results
def displayOutput(model, x, t, v, u, y, w, s):
    ...
    # Outputs external output connections
    print("_________________________________________________________________________________")
    print("Connection to External Output:")
    for i in t:
        if t[i].x >= 0.5:
            print(f"Connect {i} to external output")
    ...
    print("_________________________________________________________________________________")
    print("Connection to External Inputs:")
    for l, i, n in u:
        if u[l,i,n].x >= 0.5:
...
def scenarioAnalysis(model, x, t, v, u, y, w, s):
    ...
    for j in scenarios:
        ...
        print("Output","=",externalOutputValues[j])
...
def main():
    model, x, t, v, u, y, w, s = solveModel()
    displayOutput(model, x, t, v, u, y, w, s)
